 ...
 *
 * Hardening: fetch retries with exponential backoff + 12s timeout.
 * order_seen includes notionalUsd for market note + ML path.
 */
...
function estimateNotionalUsd(inputToken, inputStart, outputToken, outputStart) {
  const inTok = (inputToken || "").toLowerCase();
  const outTok = (outputToken || "").toLowerCase();
  const inAmt = Number(inputStart);
  const outAmt = Number(outputStart);
  if (inTok === BASE_USDC && Number.isFinite(inAmt)) return inAmt / 1e6;
  if (outTok === BASE_USDC && Number.isFinite(outAmt)) return outAmt / 1e6;
  if (inTok === BASE_WETH && Number.isFinite(inAmt)) return (inAmt / 1e18) * 3000;
  if (outTok === BASE_WETH && Number.isFinite(outAmt)) return (outAmt / 1e18) * 3000;
  return null;
}
...
function upsertTracked(map, snap) {
  const progress = decayProgressBps(snap.decayStartTime, snap.decayEndTime, snap.observedAt);
...
function summarizeLive(live, cycle, raw, newOrders, goneOrders, fetchError) {
  ...
  const pairCounts = {};
  const decayBuckets = { pre: 0, early: 0, mid: 0, late: 0, finished: 0, unknown: 0 };
  let exclusiveLive = 0;
  for (const t of live.values()) {
    const bucket = sizeBucket(t.inputToken, t.inputStart, t.outputToken, t.outputStart);
...
async function fetchOpenOrders(retries = 3) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      return { rawList, requestUrl, attempts: attempt + 1 };
    } catch (err) {
      lastErr = err;
      if (attempt < retries) {
...
async function cycle() {
  ...
  for (const raw of rawList) {
    ...
    if (isNew) {
      ...
      const notionalUsd = estimateNotionalUsd(
        snap.inputToken, snap.inputStart, snap.outputToken, snap.outputStart
      );
      await appendEvent({
        kind: "order_seen",
    ...
    } else if (tracked.lastDecayProgressBps - tracked.firstDecayProgressBps >= 2000) {
      await appendEvent({
        kind: "order_update",
        orderHash: snap.orderHash,
  ...
  for (const h of goneHashes) {
    ...
    if (tracked) {
      ...
      await appendEvent({
        ...
        sizeBucket: sizeBucket(tracked.inputToken, tracked.inputStart, tracked.outputToken, tracked.outputStart),
  ...
  cycles += 1;
  const summary = summarizeLive(live, cycles, rawList.length, newOrders, goneHashes.length, fetchError);
